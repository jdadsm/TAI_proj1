...
public:
    ...
    CopyModel(std::string& filename, int chunkSize, double threshold, std::string method): inputFile(filename), chunkSize(chunkSize),threshold(threshold), method(method){
    ...
    void writeIterationData(long iteration, double currthreshold, long totalMisses, 
        ...
        if(copy_char==""){
            ...
            if(copy_char==symbol_in){
                ...
                double prob = (1-pHit_s)*(prob_hit(hits[copy_char],tries[copy_char]-hits[copy_char])/prob_total);
                bits_it = eai(prob);
        ...
        if(logs){
            ...
            for (std::string symbol : uniqueSymbols){
                if(symbol_in==symbol){
                    ...
                    pHit = (1-pHit_s)*(prob_hit(hits[symbol],tries[symbol]-hits[symbol]))/prob_total;
                    bits = eai(pHit);
                }
        ...
        if(logs){
            ...
            + "current threshold:" + std::to_string(currthreshold) + "\n"
    ...
    long copy(long pointer_in, long length,std::string& chunk_in,long& last,bool logs,std::string th_method){
        ...
        std::string copyChar = "";
        while( (currthreshold<threshold || localTries <=10) && (pointer+1 < length)){
            std::string predictionChar = std::string(1,data[pointer]);
            tries[predictionChar] += 1;
            // before a the first sequence repetition
            if(copyPointer == -1){
                if(beforeRepetition(copyPointer,chunk,pointer,localMisses, currthreshold, localTries,
                 predictionChar, copyChar,th_method,last_ten,logs)) continue;
            }
            //after
            //printf("b-%ld",pointer);
            
            afterRepetition(copyPointer,chunk,pointer,localMisses, currthreshold, localTries,
    ...
    double& currthreshold, long& localTries, std::string& predictionChar,std::string& copyChar,
    std::string th_method,std::queue<long>& last_ten,bool logs){

        if (hashTable.find(chunk) == hashTable.end()) {
            hashTable[chunk] = pointer;
            writeIterationData(pointer,currthreshold,localMisses,localTries,predictionChar,copyChar,true,logs);
            localMisses+=1;
            localTries+=1;
            //printf("a-%ld",pointer);
            pointer+=1;
            ...
            if (th_method == "last ten"){
                ...
                last_ten.push(0);
                currthreshold = static_cast<double>(sumElements(last_ten))/(last_ten.size()); 
            }else{
                currthreshold = static_cast<double>(localMisses)/(localTries); 
            }
    ...
    void afterRepetition(long& copyPointer,std::string& chunk,long& pointer,long& localMisses,double& currthreshold, long& localTries,
     std::string& predictionChar,std::string& copyChar,std::string th_method,std::queue<long>& last_ten,bool logs){
        ...
        if(copyChar == predictionChar){
            ...
            writeIterationData(pointer,currthreshold,localMisses,localTries,predictionChar,copyChar,false,logs);
            if (th_method == "last ten"){
                if(last_ten.size()==10)last_ten.pop();
                last_ten.push(1);
                currthreshold = static_cast<double>(sumElements(last_ten))/(last_ten.size()); 
